...
async def create_topic(topic: TopicCreate, current_user: UserInDB = Depends(get_current_user)):
    db = await get_database()
    topic_dict = topic.model_dump()
    topic_dict["user_id"] = str(current_user.id)
...
async def get_topics(current_user: UserInDB = Depends(get_current_user)):
    db = await get_database()
    topics = []
    cursor = db["topics"].find({"user_id": str(current_user.id)}).sort("created_at", -1)
    async for document in cursor:
        topics.append(TopicInDB(**document))
    return topics

@router.delete("/{topic_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_topic(topic_id: str, current_user: UserInDB = Depends(get_current_user)):
    if not ObjectId.is_valid(topic_id):
        raise HTTPException(status_code=400, detail="Invalid topic ID format")
        
    db = await get_database()
    result = await db["topics"].delete_one({"_id": ObjectId(topic_id), "user_id": str(current_user.id)})
